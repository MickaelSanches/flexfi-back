...
import { LOI, ILOI } from '../models/LOI';
...
class LOIService {
  ...
  async generatePDF(loiData: ILOIFormData): Promise<string> {
    return new Promise((resolve, reject) => {
      try {
        ...
        const filename = `LOI_FlexFi_${loiData.company.replace(/\s+/g, '_')}_${timestamp}.pdf`;
        const outputPath = path.join(__dirname, '../../uploads/loi', filename);
        
        // Get image paths
        const logoPath = path.join(__dirname, '../assets/images/Logo_-_FlexF.png');
        const bannerPath = path.join(__dirname, '../assets/images/LOI_banner_-_FlexFi.png');
        ...
        doc.text('Subject: Statement of Intent to Participate in a Pilot Partnership with FlexFi', margin, y);
        y += 20;
        ...
        doc.text(introText, margin, y, { width: doc.page.width - (margin * 2), align: 'justify' });
        y = doc.y + 10;
        
        // About FlexFi - more concise
        const aboutText = 'FlexFi is a crypto-native Buy Now, Pay Later solution, allowing consumers to split purchases into multiple installments without fees, while earning crypto-based rewards.';
        doc.text(aboutText, margin, y, { width: doc.page.width - (margin * 2), align: 'justify' });
        y = doc.y + 15;
        ...
        doc.fontSize(10).fillColor('#333333');
        doc.text('This non-binding letter of intent aims to demonstrate real market interest, support FlexFi\'s development, and position our company as a priority partner for the pilot phase.', margin, y, { width: doc.page.width - (margin * 2) });
        y = doc.y + 15;
        ...
        doc.fontSize(10).fillColor('#000000');
        doc.text(`Signed in ${loiData.country}, on ${formattedDate}`, margin, y);
        y = doc.y + 15;
        ...
        if (loiData.signature) {
          const signatureData = loiData.signature.replace(/^data:image\/png;base64,/, '');
          const signatureBuffer = Buffer.from(signatureData, 'base64');
          
          // Save signature as temporary file to load into PDF
          const signatureTempPath = path.join(__dirname, '../../uploads/loi/temp_signature.png');
        ...
        if (logoExists) {
          doc.image(logoPath, doc.page.width - 120, doc.page.height - 80, { width: 70 });
        } else {
   ...
   */
  private addBulletPoint(doc: PDFKit.PDFDocument, text: string, x: number, y: number): number {
    const bulletX = x + 5;
    const textX = x + 15;
    
    doc.fontSize(10).fillColor('#333333');
    doc.text('•', bulletX, y);
    doc.text(text, textX, y, { width: doc.page.width - textX - 50, continued: false });
