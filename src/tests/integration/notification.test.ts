...
app.get('/api/notifications/unread/count', async (req: any, res: any) => {
  try {
    ...
    const { total } = await notificationService.getUserNotifications(userId, 0, 0, true);
...
app.put('/api/notifications/:id/read', async (req: any, res: any) => {
  try {
    ...
    const notification = await notificationService.markNotificationAsRead(req.params.id, userId);
...
describe('Notification System Integration Test', () => {
  ...
  test('KYC status change creates a notification and can be accessed through API', async () => {
    ...
    expect(notifications[0].message).toBe('Your KYC verification has been approved. You now have full access to FlexFi services.');
    ...
    const updatedNotification = await NotificationModel.findById(notifications[0]._id);
