...
async function exportData() {
  try {
    ...
    // Connexion à MongoDB
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/flexfi');
    ...
    const userHeaders = ['id', 'email', 'firstName', 'lastName', 'authMethod', 'kycStatus', 'walletCount', 'createdAt'];
    ...
    const walletHeaders = ['id', 'userId', 'publicKey', 'type', 'hasDelegation', 'createdAt'];
    ...
    fs.writeFileSync(path.join(EXPORT_DIR, `wallets_${dateStr}.csv`), walletsCsv);
